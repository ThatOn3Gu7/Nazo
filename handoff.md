...
## Update screen: curated notes instead of the raw commit log

The in-app update screen rendered the GitHub release body **verbatim**, so
players saw literal `<details>`/`<summary>` tags, commit hashes, and every
`chore:`/`ci:`/`docs:` commit alongside real features.

Fixed at both ends.

**1. `gen_release_notes.py` now writes two audiences into one body.**

- A curated **What's New** section wrapped in `<!--NAZO_NOTES_START-->` /
  `<!--NAZO_NOTES_END-->` (HTML comments, invisible on GitHub).
- The full commit log after it, inside a collapsed `<details>` block, so
  developers lose nothing.

Only `feat:` -> **New** and `fix:` -> **Fixed** reach players. Everything else
(docs/chore/ci/style/refactor/test/build) is dropped. Two further filters keep
internal work out:

- a keyword list (compile, imports, typo, lint, workflow, handoff, @Composable...)
- any subject containing a camelCase/PascalCase identifier — `sparkleStyle`,
  `OnboardingScreen` — which reliably means the subject is written in code
  terms, not player terms.

Entries are de-duplicated case-insensitively and sentence-cased.

**Explicit control via a commit trailer** (always overrides the filters):

    Release-Note: Meteor shower effect on the Generate button
    Release-Note: skip

Use these when a subject is accurate for developers but wrong for players.
This is the reliable lever — the regex filters are only a sensible default.

**2. `UpdateChecker.playerFacingNotes()` extracts and cleans that section.**

Keeps only the marked block, then converts the leftover Markdown for a plain
`Text`: headings flattened, `-`/`*` bullets to `•`, `**bold**` and backticks
unwrapped, blank runs collapsed.

Releases published *before* the markers existed (v8.0 and earlier) have none, so
there is a fallback: keep only unindented commit-subject lines and strip the
hash prefix. Indentation is the reliable signal there, since every `<details>`
body is indented.

Note the regexes are top-level `val`s (compiled once) and the replacement string
is `"\$1"` — a bare `"$1"` in Kotlin is a string template and does not compile.

Verified with 15 classifier cases plus an end-to-end render of both the new and
legacy body shapes.

### How to test it live

Full verification needs a release **after** this change (v9.0+), since v8.0's
notes were generated by the old script.

1. **Right now (legacy path):** About -> Check for updates against v8.0.
   **Expected:** a clean `•` list of commit subjects — no `<details>`,
   no `<summary>`, no 7-character hashes, no indented description text.
2. **After the next release:** the same screen should show only
   *What's New / New / Fixed* with short sentences, and nothing tagged
   `chore:`, `ci:` or `docs:`.
3. **Check the GitHub release page** for that version: the curated list appears
   at the top and *Full commit log* is a collapsed section below it.
4. **Exercise the trailer:** add `Release-Note: skip` to a `feat:` commit and
   confirm it is absent from the in-app list but still present in the
   collapsed commit log.

---

## [2026-09-10 14:20] feat: landscape-mode support across the app

The app had no orientation handling at all: `AndroidManifest.xml` declared no
`screenOrientation`, there were no `-land` resource folders, and every screen
was a single portrait column. Rotating simply stretched that column, so lines
ran edge to edge and the quiz options fell below the fold.

Landscape branching is deliberately kept in ONE place — the new
`ui/components/Orientation.kt` — rather than scattered `LocalConfiguration`
checks. Public API:

- `isLandscape(): Boolean` — reads `LocalConfiguration.orientation`.
- `NazoReadableWidth = 600.dp` — width cap for list-style screens.
- `NazoRailWidth = 84.dp` — width screens must reserve for the nav rail.
- `NazoReadableColumn(...)` — full width in portrait, capped and centred in
  landscape.
- `NazoAdaptivePanes(...)` — `Row` in landscape, `Column` in portrait.

Design rule (also in the file's KDoc): vertical-list screens use
`NazoReadableColumn`; screens with a dominant visual use two panes.

What changed per screen:

- **Navigation** (`ui/components/NazoBottomNav.kt`): in landscape the bottom bar
  becomes a right-edge vertical rail. Vertical space is the scarce axis when
  rotated, so a bottom bar costs the most valuable pixels. The rail honours the
  existing floating/docked preference (floating = rounded pill with shadow and
  border; docked = left-rounded solid surface) and keeps
  `blockTouchThrough()`. Unlike the portrait pill, the rail shows BOTH labels
  always — there is room, and an icon-only rail is ambiguous.
- **App shell** (`ui/NazoApp.kt`): the nav is aligned `CenterEnd` in landscape.
  Screens that show the rail (Home, Settings and its sub-screens) inset their
  content by `NazoRailWidth`; full-screen screens such as the quiz keep the full
  width. The screen `when` was extracted into a local `renderScreen` composable
  lambda so the settings detail pane reuses it rather than duplicating
  navigation.
- **Settings** (landscape only): a list-detail layout. The settings list stays
  on the left (weight 0.42) while the selected sub-screen renders on the right
  (weight 0.58), so moving between Appearance, Statistics, Backup and About is
  one tap instead of back-then-forward. `Screen.asSettingsDetail()` decides
  which screens may render in the detail pane; everything else stays
  full-screen. Before a section is chosen the pane shows a short hint. The rail
  and its inset persist while a sub-screen is open.
- **Active quiz** (`ui/screens/ActiveQuizScreen.kt`): question card on the left
  with the explanation appearing beneath it (that pane has spare room once
  answered), and the four options on the right where all of them are reachable
  without scrolling. Each pane scrolls independently, and the page-level
  `verticalScroll` is dropped in landscape so the two scrollers cannot fight the
  same drag. Header and spacer chrome is compressed (20/16/24dp → 8/8/10dp).
- **Results** (`ui/screens/QuizCompleteScreen.kt`): score ring on the left with
  the new-record badge and daily bonus in the same pane; the time / accuracy /
  difficulty cards and the action buttons on the right. The page scroll is
  likewise dropped in favour of per-pane scrolling.
- **Guessing game** (`modes/guessing_game/GuessingPlayScreen.kt`): mystery image
  on the left, hint row, answer input and reveal card on the right, so the input
  is not pushed under the keyboard. The image card's fixed `300.dp` height —
  taller than a landscape content area — is capped at 62% of window height.
- **Home** (`ui/screens/HomeScreen.kt`): capped to `NazoReadableWidth` and
  centred in landscape; the 96dp bottom nav clearance drops to 20dp because the
  nav is a rail there.

Files: `ui/components/Orientation.kt` (new), `ui/components/NazoBottomNav.kt`,
`ui/NazoApp.kt`, `ui/screens/ActiveQuizScreen.kt`,
`ui/screens/QuizCompleteScreen.kt`, `ui/screens/HomeScreen.kt`,
`modes/guessing_game/GuessingPlayScreen.kt`.

Note: CI only compiles. A layout can compile perfectly and still look wrong, so
every item below needs eyes on a real rotated device.

### How to test it live

Precondition for all of it: auto-rotate ON in the system quick settings.

1. **Nav rail.** Open the app on Home, rotate to landscape. The bottom bar
   should move to the right edge as a vertical rail, Home above Settings, each
   icon with its label underneath. Tap Settings, then Home: the pill should
   slide between the two, animated, not jump.
2. **Rail style follows the preference.** Portrait → Settings → Appearance →
   toggle the floating nav bar preference. Rotate. Floating ON = a rounded pill
   detached from the edge with a shadow; OFF = a solid panel flush to the right
   edge, rounded on its left side only. Both should sit clear of the gesture
   bar.
3. **Rail does not swallow taps.** In landscape, put a Home card behind the rail
   area and tap the rail's empty background between the two items. Nothing
   underneath should react.
4. **Home readable width.** Rotate on Home. Cards should be centred in a column
   roughly 600dp wide with empty space either side, NOT stretched to the rail.
   Scroll to the bottom: the last element should be reachable and not hidden.
5. **Quiz two-pane.** Start any quiz, rotate mid-question. Question card left,
   all four options right, all four visible without scrolling. Tap an answer:
   the explanation appears under the QUESTION on the left, not below the
   options.
6. **Quiz independent scrolling.** In the same landscape question, drag the left
   pane up and down — only the question/explanation should move. Then drag the
   right pane — only the options should move. Neither should drag the whole
   page, and one should never scroll the other.
7. **Quiz chrome.** Compare the header in portrait vs landscape: timer circle,
   hearts, progress bar and lifelines should all still be there in landscape,
   just tighter vertically, leaving the question and options the bulk of the
   height.
8. **Results two-pane.** Finish a quiz, rotate on the results screen. Score ring
   on the left; time, accuracy and difficulty cards plus Play Another / Review /
   Share on the right. Every button should be tappable without scrolling.
9. **Record badge placement.** Set a new personal best (easiest: one question on
   Easy, answer correctly), rotate on results. The NEW RECORD badge should be in
   the LEFT pane directly under the ring, not floating in the right column.
10. **Settings list-detail.** Rotate on Settings. The settings list stays on the
    left; the right side shows "Pick a section on the left". Tap Appearance —
    it fills the right pane while the list REMAINS visible. Tap Statistics from
    that same list without going back: the right pane should cross-fade to
    Statistics.
11. **Rail persists in list-detail.** With Appearance open in the right pane,
    confirm the nav rail is still on the right edge with Settings highlighted,
    and that neither pane slides underneath it.
12. **Settings back behaviour.** With a sub-screen open in landscape, press
    system back once: it should return to the placeholder/list state, not exit
    to Home.
13. **Portrait settings unchanged.** Rotate back to portrait from a settings
    sub-screen. It must be a normal full-screen page with the nav bar at the
    bottom, exactly as before — no leftover split.
14. **Guessing game split.** Start a guessing round, rotate. Mystery image left,
    hint pill + answer input + submit right.
15. **Guessing image height.** In that landscape round the image must fit inside
    the visible area — no cropping against the bottom edge and no need to
    scroll the left pane to see the whole card.
16. **Guessing keyboard.** In landscape with the autocomplete input (Hard or
    Otaku Master), tap the text field. The field and its suggestions should stay
    visible above the keyboard.
17. **Rotate mid-state.** Rotate during a running quiz timer and during a
    guessing reveal animation. The timer must keep counting and the reveal must
