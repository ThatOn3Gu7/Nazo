...
class UpdateCheckWorker(context: Context, params: WorkerParameters) : CoroutineWorker(context, params) {
    ...
    override suspend fun doWork(): Result {
        ...
        if (latest.tag == prefs.lastNotifiedVersion) return Result.success()
        if (!canNotify()) return Result.success()
        notifyUpdateAvailable(latest)
        prefs.setLastNotifiedVersion(latest.tag)
    ...
    private fun notifyUpdateAvailable(release: GitHubRelease) {
        ...
        val notification = NotificationCompat.Builder(context, CHANNEL_ID)
            ...
            .setContentTitle("Nazo ${release.tag} is available")
    ...
    companion object {
        private const val CHANNEL_ID = "nazo_updates"
